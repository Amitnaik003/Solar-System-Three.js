...
export class Planet {
  ...
  updateOrbitRotation() {
    const multipliers = window.planetSpeedMultipliers || [];
    const speedMultiplier = multipliers[this.index] || 1; // Default to 1x speed
    if (this.orbitRotationDirection === "clockwise") {
      this.group.rotation.y -= this.orbitSpeed * speedMultiplier;
